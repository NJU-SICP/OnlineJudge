...
def parse_input(command_input=None):
    ...
    testing.add_argument('--query', action='store_true',
    ...
    server.add_argument('--server', type=str,
                        default='sicp-api.njujb.com:28300',
...
def main():
    ...
    if args.version:
        ...
        did_update = software_update.check_version(args.insecure,
                                                   args.server,
                                                   client.__version__,
                                                   client.FILE_NAME,
                                                   timeout=10,
    ...
    try:
        ...
        if args.query:
            assign.protocol_map['query'].run(None)
            assign = None
            exit(0)

        if args.submit:
            assign.protocol_map['submit'].run(None)
            assign = None
            exit(0)
        ...
        if not args.no_update and not args.local:
            try:
                software_update.check_version(args.insecure,
                                              args.server,
                                              client.__version__,
                                              client.FILE_NAME)
            except KeyboardInterrupt:
