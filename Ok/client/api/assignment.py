...
class Assignment(core.Serializable):
    ...
    def authenticate(self, force=False, inline=False, nointeract=False):
        ...
        if not inline:
            return auth.authenticate(self.cmd_args, force=force, nointeract=nointeract)
        else:
            return auth.notebook_authenticate(self.cmd_args, force=force, nointeract=nointeract)

    def get_student_email(self):
        return auth.get_student_info(self.cmd_args)

    def get_identifier(self):
        return auth.get_identifier(self.cmd_args)
    ...
    def _print_header(self):
        if getattr(self.cmd_args, 'autobackup_actual_run_sync', False):
            return
        format.print_line('=')
        print('Assignment: {} {}'.format(self.endpoint, self.name))
